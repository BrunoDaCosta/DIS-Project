...
static float Kw = 0.5;
static float Kb = 1;
static float Ku_mult = 0.2;
static float Kw_mult = 0.5;
static float Kb_mult = 1;
...
void compute_wheel_speeds(int nsl, int nsr, float *msl, float *msr) {
	...
	{
      	    w = Kw * range * sinf(bearing);
	     counter_kb+=1;
            }
            else
            {
      	    w = Kw * range * sinf(bearing) - Kb * sinf(delta);
            }
